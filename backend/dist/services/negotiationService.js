...
export async function initiateNegotiation(input) {
    if (!(input.quantity > 0))
        throw httpError('Quantity must be greater than zero', 400);
    if (!(input.proposedPrice > 0))
        throw httpError('Proposed price must be greater than zero', 400);
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        const stockRes = await client.query(`SELECT id, stockholder_id, custom_product_name, price_per_unit
       FROM public.stockholder_inventory
       WHERE id = $1 AND is_available = true`, [input.stockId]);
        const stock = stockRes.rows[0];
        if (!stock)
            throw httpError('Product not found or unavailable', 404);
        ...
        await client.query(`UPDATE public.negotiations
       SET status = 'REJECTED', updated_at = now()
       WHERE shop_owner_id = $1 AND stock_id = $2 AND status IN ('PENDING', 'COUNTERED')`, [input.shopOwnerId, input.stockId]);
        const result = await client.query(`INSERT INTO public.negotiations
...
export async function listShopOwnerNegotiations(shopOwnerId) {
    const { rows } = await db.query(`${SELECT_BASE} WHERE n.shop_owner_id = $1 ORDER BY n.updated_at DESC LIMIT 100`, [shopOwnerId]);
    return rows.map(mapRow);
}
/** All bargains addressed to a supplier. */
export async function listSupplierNegotiations(stockholderId) {
    const { rows } = await db.query(`${SELECT_BASE} WHERE n.stockholder_id = $1 ORDER BY n.updated_at DESC LIMIT 100`, [stockholderId]);
    return rows.map(mapRow);
}
/**
...
export async function counterNegotiation(input) {
    if (!(input.proposedPrice > 0))
        throw httpError('Counter price must be greater than zero', 400);
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        const { rows } = await client.query(`SELECT * FROM public.negotiations WHERE id = $1 FOR UPDATE`, [input.negotiationId]);
        const neg = rows[0];
        if (!neg)
            throw httpError('Negotiation not found', 404);
       ...
       WHERE id = $3`, [input.proposedPrice, input.counterMessage ?? null, input.negotiationId]);
        await client.query(`INSERT INTO notifications (user_id, title, subtitle, type)
       VALUES ($1, $2, $3, 'negotiation')`, [
            neg.shop_owner_id,
...
export async function respondToNegotiation(input) {
    ...
    try {
        await client.query('BEGIN');
        const { rows } = await client.query(`SELECT n.*, si.unit, si.price_per_unit
       FROM public.negotiations n
       JOIN public.stockholder_inventory si ON si.id = n.stock_id
       WHERE n.id = $1 FOR UPDATE`, [input.negotiationId]);
        const neg = rows[0];
        if (!neg)
            throw httpError('Negotiation not found', 404);
        ...
        if (input.action === 'decline') {
         ...
         WHERE id = $1`, [input.negotiationId]);
            await client.query(`INSERT INTO notifications (user_id, title, subtitle, type)
         VALUES ($1, $2, $3, 'negotiation')`, [
                neg.stockholder_id,
                'Bargain declined',
        ...
        if (neg.last_offered_by !== 'supplier') {
            throw httpError('Waiting for the supplier to respond before you can accept', 409);
        }
        if (!['PENDING', 'COUNTERED'].includes(neg.status)) {
            throw httpError(`Negotiation already ${neg.status}`, 409);
        }
        const orderId = await createOrderFromNegotiation(client, neg, input.shopOwnerId);
        await client.query('COMMIT');
        const updated = await db.query(`${SELECT_BASE} WHERE n.id = $1`, [
            input.negotiationId,
        ]);
 ...
 */
async function createOrderFromNegotiation(client, neg, acceptedByUserId) {
    // Re-validate live stock availability at the negotiated price
    const stockRes = await client.query(`SELECT quantity_available, is_available, unit
     FROM public.stockholder_inventory WHERE id = $1`, [neg.stock_id]);
    const stock = stockRes.rows[0];
    ...
    const ownerRes = await client.query(`SELECT address FROM public.users WHERE id = $1`, [neg.shop_owner_id]);
    const totalAmount = Number(neg.current_proposed_price) * Number(neg.quantity);
    const orderRes = await client.query(`INSERT INTO public.orders
    ...
    await client.query(`UPDATE public.negotiations
     ...
     WHERE id = $3`, [orderRes.rows[0].id, acceptedByUserId, neg.id]);
    const counterpartyId = acceptedByUserId === neg.stockholder_id ? neg.shop_owner_id : neg.stockholder_id;
    await client.query(`INSERT INTO notifications (user_id, title, subtitle, type)
...
export async function sendNegotiationMessage(input) {
    ...
    try {
        ...
        if (!neg)
            throw httpError('Negotiation not found', 404);
        const expectedSender = input.senderType === 'SHOP_OWNER' ? neg.shop_owner_id : neg.stockholder_id;
        ...
        await client.query(`INSERT INTO public.negotiation_messages
         (negotiation_id, sender_type, sender_id, message, offered_price)
       VALUES ($1, $2, $3, $4, $5)`, [input.negotiationId, input.senderType, input.senderId, finalText, hasOffer ? Number(input.offeredPrice) : null]);
        if (hasOffer) {
            await client.query(`UPDATE public.negotiations
         ...
         WHERE id = $3`, [Number(input.offeredPrice), input.senderType === 'SHOP_OWNER' ? 'shop_owner' : 'supplier', input.negotiationId]);
        }
        // Notify counterparty
        const counterpartyId = input.senderType === 'SHOP_OWNER' ? neg.stockholder_id : neg.shop_owner_id;
        const preview = hasOffer && !text
            ? `৳${Number(input.offeredPrice).toFixed(2)} offer`
            : finalText.slice(0, 80);
        await client.query(`INSERT INTO notifications (user_id, title, subtitle, type)
       VALUES ($1, $2, $3, 'negotiation')`, [counterpartyId, 'Negotiation message', preview]);
        await client.query('COMMIT');
        const updated = await db.query(`${SELECT_BASE} WHERE n.id = $1`, [
            input.negotiationId,
        ]);
 ...
 */
export async function getNegotiationThread(negotiationId, requesterId) {
    const { rows } = await db.query(`${SELECT_BASE} WHERE n.id = $1`, [negotiationId]);
    const neg = rows[0];
    if (!neg)
        throw httpError('Negotiation not found', 404);
    ...
    const { rows: msgRows } = await db.query(`SELECT m.*, COALESCE(u.business_name, u.full_name, 'User') AS sender_name
     FROM public.negotiation_messages m
     LEFT JOIN public.users u ON u.id = m.sender_id
     WHERE m.negotiation_id = $1
     ORDER BY m.created_at ASC`, [negotiationId]);
    const messages = msgRows.map((m) => ({
        ...
        message: m.message,
        offeredPrice: m.offered_price != null ? Number(m.offered_price) : null,
...
export async function finalizeNegotiation(input) {
    ...
    try {
        ...
        if (!neg)
            throw httpError('Negotiation not found', 404);
        if (neg.shop_owner_id !== input.requesterId &&
        ...
        const requesterType = neg.shop_owner_id === input.requesterId ? 'shop_owner' : 'supplier';
        if (neg.last_offered_by === requesterType) {
            throw httpError('You cannot accept your own offer — wait for a counter-offer', 409);
        }
        const orderId = await createOrderFromNegotiation(client, neg, input.requesterId);
        await client.query('COMMIT');
        const updated = await db.query(`${SELECT_BASE} WHERE n.id = $1`, [
            input.negotiationId,
        ]);
