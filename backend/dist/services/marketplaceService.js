...
export function mapMarketplaceProductRow(row) {
    return {
...
export async function searchMarketplace(params) {
    const { query, shopLat, shopLng, category, maxDistance = 50, sortBy = 'distance', limit = 50, offset = 0, } = params;
  ...
  `;
    const queryParams = [safeLat, safeLng];
    let paramIdx = 3;
 ...
 */
export async function getProductDetail(stockId, shopLat, shopLng) {
    const safeLat = shopLat ?? 23.777176;
    const safeLng = shopLng ?? 90.399451;
    const haversineExpr = `
    CASE
  ...
  `;
    const { rows } = await db.query(sql, [safeLat, safeLng, stockId]);
    if (rows.length === 0)
        return null;
 ...
 */
export async function getProductsByCategory(category, shopLat, shopLng, limit = 50, offset = 0) {
    const safeLat = shopLat ?? 23.777176;
    const safeLng = shopLng ?? 90.399451;
    const haversineExpr = `
    CASE
