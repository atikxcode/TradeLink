 ...
 */
export async function acceptOrder(orderId, supplierId) {
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        const { rows } = await client.query(`SELECT id, supplier_id, status, shop_owner_id, product_name
 ...
 */
export async function declineOrder(orderId, supplierId) {
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        const { rows } = await client.query(`SELECT id, supplier_id, status, shop_owner_id, product_name
       FROM orders WHERE id = $1 FOR UPDATE`, [orderId]);
        ...
        // Restore stock
        const { rows: orderDetail } = await client.query(`SELECT product_name, quantity, supplier_id FROM orders WHERE id = $1`, [orderId]);
        if (orderDetail[0]) {
            await client.query(`UPDATE stockholder_inventory
         SET quantity_available = quantity_available + $1
         WHERE stockholder_id = $2 AND custom_product_name ILIKE '%' || $3 || '%'
         LIMIT 1`, [orderDetail[0].quantity, orderDetail[0].supplier_id, orderDetail[0].product_name]);
        }
        // Notify shop owner
        await client.query(`INSERT INTO notifications (user_id, title, subtitle, type)
 ...
 */
export async function markOutOfDelivery(orderId, supplierId) {
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        const { rows } = await client.query(`SELECT id, supplier_id, status, shop_owner_id, product_name
 ...
 */
export async function confirmDeliveryWithOtp(orderId, supplierId, otpInput) {
    const client = await db.connect();
    try {
        await client.query('BEGIN');
        const { rows } = await client.query(`SELECT id, supplier_id, shop_owner_id, status, delivery_otp, product_name, quantity, inventory_id
       FROM orders WHERE id = $1 FOR UPDATE`, [orderId]);
        const order = rows[0];
        if (!order)
            throw httpError('Order not found', 404);
        ...
        // Mark delivered
        await client.query(`UPDATE orders SET status = 'delivered', updated_at = now() WHERE id = $1`, [orderId]);
        // Mark OTP as verified
        await client.query(`UPDATE otps SET is_verified = true WHERE order_id = $1`, [orderId]);
        ...
        const invRows = await client.query(`SELECT COALESCE(
         ...
         (SELECT si.id FROM public.stockholder_inventory si
          ...
          LIMIT 1)
       ) AS id`, [orderId, order.supplier_id, order.product_name]);
        ...
        if (linkedInventoryId) {
         ...
         WHERE id = $2`, [order.quantity, linkedInventoryId]);
        ...
        const invResult = await client.query(`SELECT id FROM public.stockholder_inventory
...
//# sourceMappingURL=orderLifecycleService.js.map