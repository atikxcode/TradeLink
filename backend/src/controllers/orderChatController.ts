...
function mapOrderChat(row: any): OrderChatDto {
  return {
    id: row.id,
 ...
 */
export const getOrderChatMessagesHandler = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const orderRes = await client.query(
        `SELECT shop_owner_id, supplier_id, delivery_man_id 
      ...
      let chatRes = await client.query(
        `${ORDER_CHAT_SELECT} WHERE c.order_id = $1 FOR UPDATE`,
        [orderId]
      );

      if (chatRes.rows.length === 0) {
        await client.query(
          `INSERT INTO public.order_chats (order_id) VALUES ($1)`,
          [orderId]
        );
        chatRes = await client.query(
          `${ORDER_CHAT_SELECT} WHERE c.order_id = $1`,
          [orderId]
        );
      }
      ...
      const msgsRes = await client.query(
        `SELECT m.*, COALESCE(u.business_name, u.full_name, 'User') AS sender_name, u.last_active_at
 ...
 * Body: { textContent }
 */
export const sendOrderChatMessageHandler = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const senderId = req.userId!;
    const role = (req.role ?? '').toLowerCase();
    
    let senderType = 'SHOP_OWNER';
    if (role === 'supplier') senderType = 'SUPPLIER';
    if (role === 'delivery_man' || role === 'delivery') senderType = 'DELIVERY_RIDER';
    ...
    try {
      ...
      // Get or create chat
      let chatRes = await client.query(
        `SELECT id FROM public.order_chats WHERE order_id = $1 FOR UPDATE`,
        [orderId]
      );
      ...
      if (chatRes.rows.length === 0) {
        const insertChat = await client.query(
          `INSERT INTO public.order_chats (order_id) VALUES ($1) RETURNING id`,
          [orderId]
        );
      ...
      await client.query(
        `UPDATE public.order_chats SET last_message = $1, updated_at = now()
         WHERE id = $2`,
      ...
      for (const pId of otherParticipants) {
        await client.query(
          `INSERT INTO notifications (user_id, title, subtitle, type)
           VALUES ($1, $2, $3, 'order_chat')`,
          [pId, 'New message in Order Chat', text.slice(0, 80)]
        );
      }
 ...
 */
export const sendOrderChatImageHandler = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const senderId = req.userId!;
    const role = (req.role ?? '').toLowerCase();
    ...
    try {
      ...
      const inserted = await client.query(
        `INSERT INTO public.order_messages
         ...
         VALUES ($1, $2, $3, $4)
         RETURNING id, created_at`,
        [chatId, senderType, senderId, imageUrl]
      );
      ...
      for (const pId of otherParticipants) {
        await client.query(
          `INSERT INTO notifications (user_id, title, subtitle, type)
           VALUES ($1, $2, $3, 'order_chat')`,
          [pId, 'New image in Order Chat', '📷 Image']
        );
      }
