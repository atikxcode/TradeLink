 ...
 */
export async function cancelRiderRequestHandler(req: AuthRequest, res: Response) {
  try {
    const supplierId = req.userId;
    const { id: orderId } = req.params;
 ...
 */
export async function pickupOrderHandler(req: AuthRequest, res: Response) {
  try {
    const deliveryManId = req.userId;
    const { id: orderId } = req.params;

    const orderCheck = await db.query(
      `SELECT id, status, supplier_id, shop_owner_id, product_name
       FROM public.orders WHERE id = $1 AND delivery_man_id = $2`,
    ...
    await db.query(
      `INSERT INTO public.notifications (user_id, title, subtitle, type)
       VALUES ($1, 'Order Picked Up', 'The delivery man has picked up the order and is on the way.', 'order_update')`,
      [order.supplier_id]
    );
 ...
 */
export async function shopOwnerConfirmDeliveryHandler(req: AuthRequest, res: Response) {
  try {
    ...
    if (order.delivery_man_id) {
      await db.query(
        `INSERT INTO public.notifications (user_id, title, subtitle, type)
         VALUES ($1, 'Delivery Confirmed', 'The shop owner has confirmed the delivery.', 'order_update')`,
 ...
 */
export async function requestQrScanHandler(req: AuthRequest, res: Response) {
  try {
    const deliveryManId = req.userId;
    const { id: orderId } = req.params;
    ...
    await db.query(
      `INSERT INTO public.notifications (user_id, title, subtitle, type)
       VALUES ($1, 'Scan QR Code', 'The delivery rider is waiting for you to scan their QR code to complete the delivery.', 'qr_scan_request')`,
